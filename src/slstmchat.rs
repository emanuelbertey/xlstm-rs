    ...
    <B as Backend>::FloatElem: num_traits::ToPrimitive + num_traits::FromPrimitive,
{
    ...
    for i in 0..length {
        ...
        let next_token = sample_from_logits(last_logits, 0.8, 40, 0.9);

        current_tokens.push(next_token);
        generated_ids.push(next_token);
        
        // Print newline immediately if generated (optional visual feedback)
    ...
    // Reconstrucción del texto final manejando Metaspace
    let mut out = String::new();
    for &id in &generated_ids {
        if let Some(mut t) = tokenizer.id_to_token(id) {
            // Manejo de Metaspace ('▁') y saltos
            if t.contains('Ċ') {
                out.push('\n'); 
                continue; 
            }
            if t.starts_with('▁') || t.starts_with('Ġ') || t.starts_with(' ') {
                out.push(' ');
                t = t.trim_start_matches('▁').trim_start_matches('Ġ').trim_start_matches(' ').to_string();
            }
            t = t.replace("\r\n", "").replace('\r', "");
            out.push_str(&t);
        }
    ...
    // Limpieza final de espacios múltiples si es necesario, o retornar raw
    out
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let num_blocks = 1; //let num_blocks = 4;
        ...
        for epoch in 0..num_epochs {
            ...
            if epoch % 1 == 0 {
                ...
                if epoch % 1 == 0 {
                    ...
                    let seed = tokenizer.decode(&seed_tokens).replace('▁', " ").replace('Ġ', " ");
